...
import handmadeguns2.gui.BlockContainerInventory_Crafter2;
...
public class TileEntity_Crafter2 extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntity_Crafter2.class, new String[] {"Items"}));
    }
    ...
    {
    	{
            ...
            if(mod_HandmadeGuns2.gun_item[sell_id] != null) {
            	if(!eme.isEmpty() && eme.getItem() == Items.EMERALD
            			&& !iron.isEmpty() && iron.getItem() == Items.IRON_INGOT
            			&& !red.isEmpty() && red.getItem() == Items.REDSTONE) {
            		if(eme.getCount() >= mod_HandmadeGuns2.gun_eme[sell_id]
            				&& iron.getCount() >= mod_HandmadeGuns2.gun_iron[sell_id]
            						&&red.getCount() >= mod_HandmadeGuns2.gun_red[sell_id]) {
            			if(this.furnaceItemStacks.get(3).isEmpty() && cansell) {
            				eme.shrink(mod_HandmadeGuns2.gun_eme[sell_id]);
            				iron.shrink(mod_HandmadeGuns2.gun_iron[sell_id]);
            				red.shrink(mod_HandmadeGuns2.gun_red[sell_id]);
            				cansell = false;
            			}else {
            				this.furnaceItemStacks.set(3, new ItemStack(mod_HandmadeGuns2.gun_item[sell_id], 1));
            				cansell = true;
            			}
            		}else {
    ...
    {
        return new BlockContainerInventory_Crafter2(playerInventory, this);
    }
