...
import handmadeguns2.gui.BlockContainerInventory_Handind2;
import handmadeguns2.network.HMGMessageKeyPressed2_Client;
...
public class TileEntity_Handind2 extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntity_Handind2.class, new String[] {"Items"}));
    }
    ...
    public void update()
    {
    	...
    	if(this.world.getBlockState(this.pos).getBlock() == mod_HandmadeGuns2.hmg2_handing2) {
        	id = 1;
        }
        else if(this.world.getBlockState(this.pos).getBlock() == mod_HandmadeGuns2.hmg2_handing3) {
        	id = 2;
        }
        else {
        	id = 0;
    	...
    	if(!this.world.isRemote) {
    		if(!this.furnaceItemStacks.get(0).isEmpty()) {
    			...
    			HMGPacketHandler2.INSTANCE2.sendToAll(new HMGMessageKeyPressed2_Client(3, 
    		...
    		if(!this.furnaceItemStacks.get(1).isEmpty()) {
    			...
    			HMGPacketHandler2.INSTANCE2.sendToAll(new HMGMessageKeyPressed2_Client(4, 
    					this.getPos().getX(), this.getPos().getY(), this.getPos().getZ()
    					,item.getIdFromItem(item)));
    		}
    		if(!this.furnaceItemStacks.get(2).isEmpty()) {
    			Item item = this.furnaceItemStacks.get(2).getItem();
    			HMGPacketHandler2.INSTANCE2.sendToAll(new HMGMessageKeyPressed2_Client(5, 
    					this.getPos().getX(), this.getPos().getY(), this.getPos().getZ()
    					,item.getIdFromItem(item)));
    		}
    		{
    			HMGPacketHandler2.INSTANCE2.sendToAll(new HMGMessageKeyPressed2_Client(6, 
    					this.getPos().getX(), this.getPos().getY(), this.getPos().getZ()
    					,this.houkou));
    		}
    		clientgetter = true;
    ...
    {
        return new BlockContainerInventory_Handind2(playerInventory, this);
    }
