...
public class BlockGuiInventory_AttachmentMachine extends GuiContainer
{
    ...
    private final TileEntity_AttachmentMachine tile;
    
    private GuiButton next;
    private GuiButton back;

    public BlockGuiInventory_AttachmentMachine(InventoryPlayer playerInv, IInventory furnaceInv, TileEntity_AttachmentMachine t)
    {
        super(new BlockContainerInventory_AttachmentMachine(playerInv, furnaceInv));
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
    	 if (button.id == 0)
         {
    		 ...
    		 HMGPacketHandler2.INSTANCE.sendToServer(new HMGMessageKeyPressed2(5, tile.getPos().getX(), tile.getPos().getY(), tile.getPos().getZ()));
         }
    	 if (button.id == 1)
         {
    		 if(tile.sell_id > 0) {
    		 ...
    		 HMGPacketHandler2.INSTANCE.sendToServer(new HMGMessageKeyPressed2(6, tile.getPos().getX(), tile.getPos().getY(), tile.getPos().getZ()));
         }
    }
    /**
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        
        {
        	...
        	{
				ItemStack item = new ItemStack(mod_HandmadeGuns2.attachment_item[tile.sell_id]);
				if (!item.isEmpty()) {
					GL11.glColor4f(1.0F, 1.0F, 1.0F, 0.4F);
					renderitem.renderItemIntoGUI(item, k + 56, l + 17);
					GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
			...
			}
        	{
				...
				if (!item.isEmpty()) {
					...
					GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
					String d2 = String.format("%1$3d", mod_HandmadeGuns2.attachment_red[tile.sell_id]);
			...
			}
        	{
				...
				if (!item.isEmpty()) {
					...
					GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
					String d2 = String.format("%1$3d", mod_HandmadeGuns2.attachment_iron[tile.sell_id]);
