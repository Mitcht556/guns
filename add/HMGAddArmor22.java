...
public class HMGAddArmor22
{
          ...
          while((str = br.readLine()) != null){  // 1行ずつ読み込む
              ...
              {//1
        			  if(type[0].equals("ArmorTexture")){
        				  armortex = "handmadeguns2:addgun/" +domain + "/addmodel/" + type[1];
            		  }
        			  if(type[0].equals("ArmorObj")){
        				  armorobj = "handmadeguns2:addgun/" +domain + "/addmodel/" + type[1];
            		  }
        			  ...
        			  if(type[0].equals("OverlayTexture")){
        				  gatex = "handmadeguns2:addgun/" +domain + "/addoverlay/" + type[1];
            		  }
        			  ...
        			  if (type[0].equals("USname")) {
                		...
                		if(type[0].equals("Armor")){
                    		...
                    		Guns.add(newgun);
                    		{
								mod_HandmadeGuns2.armor_model_item[mod_HandmadeGuns2.armor_model_id] = newgun;
								{//24/01/03
									if(itemdome && item_domain_name != null) {
										mod_HandmadeGuns2.armor_model_item_domain[mod_HandmadeGuns2.armor_model_id] = item_domain_name;
									}else {
										mod_HandmadeGuns2.armor_model_item_domain[mod_HandmadeGuns2.armor_model_id] = null;
									}
								}
								mod_HandmadeGuns2.armor_model_name[mod_HandmadeGuns2.armor_model_id] = GunName;
								++mod_HandmadeGuns2.armor_model_id;
							}
                			...
                			ArmorMaterial fuarmor = EnumHelper.addArmorMaterial("HMAArmor",
                					...
                					new int[] {hardness, hardness, hardness, hardness }, 10, null, tou);
                			GunName = type[1];
                  			Item newgun	= new ItemArmor_NewVanilla(fuarmor, slot).setUnlocalizedName(GunName)
                  					.setRegistryName(mod_HandmadeGuns2.MOD_ID,GunName)
                  					.setCreativeTab(mod_HandmadeGuns2.tabhmg);
                  			event.getRegistry().register(newgun);
  ...
  public static void Add(ItemArmor_NewObj iiitem, boolean isClient, int id){
		...
		if (isClient) {
			iiitem.obj_model = AdvancedModelLoader.loadModel(new ResourceLocation(armorobj));
    		iiitem.obj_tex = armortex;
		}
}
